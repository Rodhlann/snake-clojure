(ns snake.runner-tests
  (:require [clojure.test :refer [deftest is testing]]
            ...
            [snake.runner :refer [check-score check-tail-collision
                                  check-wall-collision update-location update-snake]]))

(def snake-up [1, 1, up, tail-length, [], [0, 0]])
(def snake-down [1, 1, down, tail-length, [], [0, 0]])
(def snake-left [1, 1, left, tail-length, [], [0, 0]])
(def snake-right [1, 1, right, tail-length, [], [0, 0]])

(def snake-up-result [1, 0, up, tail-length, [], [0, 0]])
(def snake-down-result [1, 2, down, tail-length, [], [0, 0]])
(def snake-left-result [0, 1, left, tail-length, [], [0, 0]])
(def snake-right-result [2, 1, right, tail-length, [], [0, 0]])
...
(deftest check-tail-collision-test
  (testing "no tail collision"
    (is (= (check-tail-collision [0, 0, up, tail-length, [], [0, 0]]) nil)))
  (testing "tail collision"
    (is (= (check-tail-collision [0, 0, up, tail-length, [[0, 0]], [0, 0]]) [0, 0]))))

(deftest check-wall-collision-test
  (testing "up"
    (is (= (check-wall-collision [0, -1, up, tail-length, [], [0, 0]]) true)))
  (testing "down"
    (is (= (check-wall-collision [0, (+ height 1), down, tail-length, [], [0, 0]]) true)))
  (testing "left"
    (is (= (check-wall-collision [-1, 0, left, tail-length, [], [0, 0]]) true)))
  (testing "right"
    (is (= (check-wall-collision [(+ width 1), 0, right, tail-length, [], [0, 0]]) true))))

(deftest update-snake-test
  (testing "tail-length 0"
    (is (= (update-snake 0, 1 up [0, 0, up, 0, [], [0, 0]]) [0, 1, up, 0, [], [0, 0]])))
  (testing "tail-length 1"
    (is (= (update-snake 0, 1 up [0, 0, up, 1, [], [0, 0]]) [0, 1, up, 1, [[0, 0]], [0, 0]]))))

(deftest check-score-test
  (testing "has scored"
    (let [result (check-score [1, 1, up, 0, [], [1, 1]])]
      (is
       (= (get result 3) 1)
       (not= (get result 5) [1, 1]))))
  (testing "has not scored"
    (is (= (check-score [1, 1, up, 0, [], [1, 2]]) [1, 1, up, 0, [], [1, 2]]))))